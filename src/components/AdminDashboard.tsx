...
export default function AdminDashboard({ 
  ...
  // Update lists safely from LocalStorage
  ...
  const activeLocalMembers = useMemo(() => allMembers.filter(isMemberActive), [allMembers]);
  const activeLocalPartners = useMemo(() => allPartners.filter(isPartnerActive), [allPartners]);

  const localB2BCollected = useMemo(() => {
  ...
  const localB2CCollected = useMemo(() => {
  ...
  const liveProvinceBreakdown = useMemo(() => {
  ...
  const liveMonthlyTrendData = useMemo(() => [
  ...
  // ALL ACTIONS & FUNCTIONS (DEFINED EXPLICITLY)
  ...
  return (
    <div className="bg-[#050505] min-h-screen text-white pt-6 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 pb-6 border-b border-gray-900 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-start lg:justify-end">
            <button onClick={onGoBack} className="flex items-center gap-2 px-3.5 py-2 bg-[#121212] hover:bg-[#1a1a1a] border border-gray-800 rounded-lg text-xs font-bold text-gray-300">
              <ArrowLeft className={`w-4 h-4 text-[#D30014] ${lang === "ar" ? "rotate-180" : ""}`} />
              <span>الموقع العام</span>
            </button>

            <button onClick={() => setLang(lang === "en" ? "ar" : "en")} className="flex items-center gap-2 px-3.5 py-2 bg-[#121212] hover:bg-[#1a1a1a] border border-gray-800 rounded-lg text-xs font-bold text-gray-300">
              <Languages className="w-4 h-4 text-[#D30014]" />
              <span>{t.langToggle}</span>
            </button>

            <button onClick={loadAllData} className="p-2 bg-[#121212] hover:bg-[#1f1f1f] border border-gray-800 rounded-lg text-gray-400 hover:text-white">
              <RefreshCw className="w-5 h-5" />
            </button>

            {!isViewer && (
              <button onClick={handleClearAllData} className="flex items-center gap-2 px-3.5 py-2 bg-red-500/10 hover:bg-red-600 border border-red-500/20 text-red-500 hover:text-white rounded-lg text-xs font-bold">
                <Trash2 className="w-4 h-4" />
                <span>مسح البيانات</span>
              </button>
            )}

            <button onClick={onLogout} className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-[#D30014] hover:text-white border border-red-500/20 text-red-500 rounded-lg text-xs font-bold">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-[#121212] border border-gray-800 rounded-xl p-6">
                <h3 className="text-base font-black mb-4 flex items-center gap-2">
                  <DollarSign className="w-4 h-4 text-[#D30014]" />
                  مقارنة المبالغ المحصلة مقابل المستهدفة
                </h3>
              ...
              <div className="bg-[#121212] border border-gray-800 rounded-xl p-6">
                <h3 className="text-base font-black mb-4 flex items-center gap-2">
                  ...
                  اتجاه نمو الإيرادات التراكمي (بالدينار العراقي)
                </h3>
                <div style={{ width: "100%", height: 300 }}>
                  <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="bg-[#121212] border border-gray-800 rounded-xl p-6 overflow-hidden">
              <h3 className="text-base font-black mb-4 flex items-center gap-2">
                ...
                مؤشرات الأداء المالي المفصلة حسب المحافظات الـ 19
              </h3>
                  ...
                  <tbody className="divide-y divide-gray-900 text-xs font-semibold text-gray-300">
                    {liveProvinceBreakdown.map((pb, idx) => (
                      <tr key={idx} className="hover:bg-white/[0.02]">
                        ...
                        <td className="py-3 px-4 text-center">{pb.partners} / 10</td>
                        <td className="py-3 px-4 text-center">{pb.users} / 100</td>
                        <td className="py-3 px-4 text-center text-gray-400">{pb.collectedB2B.toLocaleString()} د.ع</td>
                        <td className="py-3 px-4 text-center text-gray-400">{pb.collectedB2C.toLocaleString()} د.ع</td>
                  ...
                  {filteredMembers.map((m) => (
                    <tr key={m.id || m.cardId} className="hover:bg-white/[0.02]">
                      ...
                      <td className="py-4 px-6 text-right">
                        {!isViewer && (
                          <button onClick={() => handleDeleteMember(m.id || m.cardId)} className="p-1.5 bg-red-500/10 hover:bg-[#D30014] text-red-400 hover:text-white rounded cursor-pointer">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        )}
                      </td>
                ...
                <tbody className="divide-y divide-gray-900 text-sm font-semibold text-gray-300">
                  {filteredPartners.map((p) => (
                    <tr key={p.id || p.username} className="hover:bg-white/[0.01]">
                      ...
                      <td className="py-4 px-6 text-right">
                        {!isViewer && (
                          <button onClick={() => handleDeletePartner(p.id || p.username)} className="p-1.5 bg-red-500/5 hover:bg-[#D30014] text-red-400 hover:text-white rounded cursor-pointer">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        )}
                      </td>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-black/40 border border-gray-900 rounded-2xl p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={brandingForm.company1Name} onChange={(e) => setBrandingForm({ ...brandingForm, company1Name: e.target.value })} className="w-full px-3 py-2 bg-black border border-gray-800 rounded-lg text-white font-bold text-xs" />
                  </div>
                  <div>
                    <label className="block text-gray-400 font-bold mb-1.5 text-xs">اسم الشركة (عربي)</label>
                    <input type="text" value={brandingForm.company1NameAr} onChange={(e) => setBrandingForm({ ...brandingForm, company1NameAr: e.target.value })} className="w-full px-3 py-2 bg-black border border-gray-800 rounded-lg text-white font-bold text-xs" />
                  </div>
                </div>
                ...
                <div className="bg-black/40 border border-gray-900 rounded-2xl p-5 space-y-4">
                  ...
                  <div>
                    <label className="block text-gray-400 font-bold mb-1.5 text-xs">اسم الشركة (إنجليزي)</label>
                    <input type="text" value={brandingForm.company2Name} onChange={(e) => setBrandingForm({ ...brandingForm, company2Name: e.target.value })} className="w-full px-3 py-2 bg-black border border-gray-800 rounded-lg text-white font-bold text-xs" />
                  </div>
                  <div>
                    <label className="block text-gray-400 font-bold mb-1.5 text-xs">اسم الشركة (عربي)</label>
                    <input type="text" value={brandingForm.company2NameAr} onChange={(e) => setBrandingForm({ ...brandingForm, company2NameAr: e.target.value })} className="w-full px-3 py-2 bg-black border border-gray-800 rounded-lg text-white font-bold text-xs" />
                  </div>
                </div>
              </div>
        ...
        {activeTab === "cards" && (
          <div className="space-y-6">
            <div className="flex justify-between items-center bg-[#121212] border border-gray-800 p-6 rounded-2xl">
              ...
              {!isViewer && (
                <button onClick={() => setShowCardForm(true)} className="flex items-center gap-2 px-5 py-2.5 bg-[#D30014] hover:bg-[#b00010] text-white font-bold rounded-xl text-xs cursor-pointer">
                  ...
                  {cards.map((card: any) => (
                    <tr key={card.id}>
                      ...
                      <td className="py-4 px-6 text-right">
                        {!isViewer && (
                          <button onClick={() => {
                            const updated = cards.filter((c: any) => c.id !== card.id);
                            setCards(updated);
                            safeSetLocalStorage("byd-cards", JSON.stringify(updated));
                          }} className="p-2 bg-red-500/10 text-red-500 rounded cursor-pointer">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        )}
                      </td>
                ...
                <tbody className="divide-y divide-gray-900 text-xs font-semibold text-gray-300">
                  {viewerAccounts.map((acc) => (
                    <tr key={acc.id}>
                      ...
                      <td className="py-4 px-6 text-right">
                        <button onClick={() => handleDeleteViewerAccount(acc.id)} className="p-2 bg-red-500/10 text-red-500 rounded cursor-pointer">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </td>
                    </tr>
      ...
      {showMemberForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85">
          <div className="bg-[#121212] border border-gray-800 rounded-2xl w-full max-w-lg p-6 relative">
            <h3 className="text-lg font-black mb-4">إضافة / تعديل مشترك</h3>
      ...
      {showPartnerForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85">
          <div className="bg-[#121212] border border-gray-800 rounded-2xl w-full max-w-lg p-6 relative">
            <h3 className="text-lg font-black mb-4">إضافة / تعديل شريك</h3>
